...
const BlogArticle = ({
    ...
    const formattedDate = t('blog:dateArticle') ? `${t('blog:dateArticle')} ${DateTime.fromISO(date).toLocaleString(DateTime.DATE_FULL)}` : DateTime.fromISO(date).toLocaleString(DateTime.DATE_FULL)

    return(
        <BlogArticleStyled>
            <Link to={`/${t('common:url.blog')}/${slug}`} className="article-title"><h2 className="default-title">{title}</h2></Link>
            ...
            <div className="article-forSharing">
                ...
                {blog.socialMedia.map(({ id, name, sharer, addedSharer, picture }) => { 
                    ...
                    return (
                        <a className="article-forSharing-link" key={id} href={`${sharer}https://instantvisio.com/${t('common:url.blog')}/${slug}${checkAddedSharer}`} target="_blank" rel="noopener noreferrer">
