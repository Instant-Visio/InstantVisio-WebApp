...
const main = async () => {
    ...
    console.log(`Found ${localDirectories.length} languages: `, localDirectories)
    ...
    for (let i = 0; i < localDirectories.length; i++) {
        ...
        const items = await readCommonAndGetString(`${directoryPath}/${lang}/common.json`)
...
const readCommonAndGetString = (filePath) => {
    return new Promise(((resolve, reject) => fs.readFile(filePath, 'utf8', function (err, contents) {
        if (err) {
...
const writeTranslationsToFunctionsDir = (translations) => {
    return new Promise(((resolve, reject) =>
        fs.writeFile(outputFile, translations, "utf8", function (err, contents) {
            if (err) {
